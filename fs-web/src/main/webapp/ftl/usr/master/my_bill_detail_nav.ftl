 ...
 <#noparse>
    <div class="list-item list-two-col">
        <div class="list-item-top">
            ...
            <div class="top-right">税前收入 ¥ ${descAmtUntax}</div>
        </div>
        <div class="list-item-bottom">
            <div class="bottom-left">
                <div class="">${simpleOrderExtraInfoAndCateName}</div>
                <div class="">${timeDesc}</div>
            </div>
            <div class="bottom-right">
                <div class="item-amount">订单金额：+${descAmtOrder}</div>
                {{if amtDiscountMaster > 0}}
                <div class="item-amount">老师优惠：-${descAmtDiscountMaster}</div>
                {{/if}}
                <div class="item-amount">平台佣金：-${descAmtCommission}</div>
            </div>
        </div>
    </div>
 </#noparse>
        ...
        <div class="list">
            <div class="list-body">
				<div class="list-item list-two-col">
					<div class="list-item-top">
						<div class="top-left">
							<span style="font-weight: bold;">代缴个税</span>
						</div>
						<div class="top-right" id='totalTax'></div>
					</div>
					<div class="list-item-bottom">
						<div class="bottom-left" id='settleDate'></div>
					</div>
				</div>
            </div>
        </div>
...
$('.content').dropload({
	scrollArea: window,
	domDown: {
		domClass: 'dropload-down',  		domRefresh: '<div class ="dropload-refresh"></div>',
		...
		$.ajax({
			...
			success: function(data) {
				...
				//系统错误
					...
					$("#totalTax").html(data.body.gsDetail.descTotalTax);
					$("#settleDate").html(data.body.gsDetail.descSettleDate );
