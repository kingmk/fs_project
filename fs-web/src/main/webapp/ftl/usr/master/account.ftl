...
<link rel="stylesheet" href="${host.css}/css/my_account.css?${host.version}3">
<style type="text/css">
...
<script>
var reserveWord = "${result.body.reserveWord}";
$(function(){
	//设置click 事件 我的收入
	...
	//设置 click 事件 类目编辑
	$("#service_intro").click(function(){
		console.log("123");
		location.href = "${host.base}/usr/master/cate_intro_nav"
	});
	//设置 click 事件 我的银行卡
	...
	$("#personal_home_page").click(function(){
		...
		// location.href = "${host.base}/usr/search/master_detail?masterInfoId=";
	});
	...
	$(".header-button").click(function(){
		...
		if("ING" == serviceStatus){
			...
			$.bgmask({
				title:title,
				// text:text,
				formContent: $('<div class="reserveout"><textarea rows="5" id="reserveword" class="reserveword" name="reserveword" placeholder="请输入自定义繁忙通告，限30字">'+reserveWord+'</textarea></div>'),
				type: "form",
				buttonTxt:buttonTxt,
				buttonFn: function(){
					reserveWord = $("#reserveword").val();
	 				changeServiceStatus(_serviceStatus, reserveWord, dom);
				}
			})
			var f = function() {
				$("#reserveword").on('input propertychange', function() {
...
function changeServiceStatus(serviceStatus, reserveWord, dom){
	if(isClick){
		return;
	}
	isClick = true;
...
function updateReserveWord(reserveWord){
	if(isClick){
		return;
	}
	isClick = true;
	$.ajax({
		type: "POST",
		url: "${host.base}/usr/master/recruit/update_reserve_word",
		dataType: "json",
		data:{
			masterInfoId :${result.body.masterInfoId},
			reserveWord: reserveWord
...
<div class="header">
    <div class="header-top  <#if result.body.serviceStatus="ING">stop<#else>start</#if>">
		...
		<#if result.body.serviceStatus="ING">
			...
			<div class="update-button" style="display:none">更改通告</div>
		<#elseif result.body.serviceStatus="NOTING">
			...
			<div class="update-button">更改通告</div>
		</#if>
...
<div class="list">
    <div class="list-body">
        ...
        <div class="list-item" id = "service_intro">
            <div class="list-icon icon-service-intro"></div>
            <div class="list-label">类目编辑</div>
            <div class="list-arrow"></div>
        </div>
