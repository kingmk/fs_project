...
<link rel="stylesheet" href="${host.css}/css/my_income.css?${host.version}3">
<style>
...
<script  id='incomelist' type="text/x-jquery-tmpl">
  <div class="card-list-item">
      <div class="top">
          <div class="left">
            {{if hadSettle =="Y"}}
              <div class="data">${month}<div class="small-text">${year}</div></div>
              <div class="top-left-button"> 已结算</div>
            {{else}}
              <div class="data">&nbsp;</div>
              <div class="top-left-button unbalanced">未出账</div>
              {{/if}}
          </div>
          <div class="right">
              <div class="pretax-income">
                  <div class="tips">税前收入：<span class="income">¥${descSumAmtUntax}</span></div>
              </div>
              <div class="income-detail">
                  <div class="tips">订单总额：<span class="income">+¥${descSumAmtOrder}</span></div>
                  {{if sumAmtDiscountMaster>0}}
                  <div class="tips">老师优惠：<span class="income">-¥${descSumAmtDiscountMaster}</span></div>
                  {{/if}}
                  <div class="tips">平台佣金：<span class="income">-¥${descSumAmtCommission}</span></div>
                  <div class="tips">代缴个税：<span class="income">${descSumAmtTax}</span></div>
              </div>
          </div>
      </div>
...
<div class="header">
    ...
    <div class="header-bottom">
        <div class="bottom-item">
            ...
            <div class="text">
                <small>¥</small>
               ${funUtils.formatNumber(result.body.amtUntax/100,"###,##0.00","--")}
            </div>
        </div>
        <div class="bottom-item">
            ...
            <div class="text">
                <small>¥</small>
              ${funUtils.formatNumber(result.body.amtRefund/100,"###,##0.00","--")}
            </div>
        </div>
    </div>
</div>
